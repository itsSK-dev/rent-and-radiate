...
BEGIN
  ...
  allowed := CASE OLD.status::text
    WHEN 'pending'            THEN ARRAY['confirmed','accepted','assigned','cancelled']
    WHEN 'confirmed'          THEN ARRAY['accepted','packing','ready_for_pickup','assigned','cancelled']
    WHEN 'packing'            THEN ARRAY['ready_for_pickup','assigned','cancelled']
    ...
    WHEN 'accepted'           THEN ARRAY['confirmed','packing','ready_for_pickup','assigned','cancelled']
