...
type Rental = {
  ...
  customer: { full_name: string | null; city?: string | null } | null;
};
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [r, s, rf] = await Promise.all([
        supabase
          .from("rentals")
          .select("id,kind,status,subtotal,grand_total,commission_amount,gst_amount,discount_amount,delivery_fee,deposit,quantity,days,start_date,end_date,created_at,customer_id,product_id,product:products(title),customer:profiles!rentals_customer_id_fkey(full_name,city)")
          .eq("store_id", storeId)
